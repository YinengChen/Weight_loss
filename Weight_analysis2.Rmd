...
This data site contains the weight data of 24 patinets. Bodyweight after any intervention (TPN, Feeding tube, transplant) is not included in this data set. No. 98 only has one data point and there for is excluded. we have 23 patinets ready for analysis. 9 of them come from Natural History study. 14 of them do not come from Natural History study. No.205, No.212, and the first few data points for No.203 are excluded after discussing with Dr. Hirano. In the end we have 22 patients.

I use a mixed effect model because the number of measurment and the timing of each petient are different.
Covariates and response are listed as follow:

1. Time is the follow up period. Time(years) = date of the least measurement - date of the first measurement

1. Weight: Body weight in Kg

1. ID: Patients' id

1. Group: group 1 = patients who are not in the natural history study; group 2 = patients who are in the natural history study
...
### seperate by id ###
# Group 1 non Natural history
weight_before2_c1 = weight_before2 %>% 
  mutate(id = as.numeric(id)) %>% 
  filter(id < 200)
#View(weight_before2_c1)
# unique(weight_before2_c1$id)  # Patiend ID in group 2
# Group 2 non Natural history
weight_before2_c2 = weight_before2 %>%
  mutate(id = as.numeric(id)) %>% 
  filter(id > 200)
# View(weight_before2_c2)
# unique(weight_before2_c2$id) # Patiend ID in group 2
...
```
The Group covariate is not significant. Then fit a model without group

On average, the body weight decreases 0.84 kg per year (SE = 0.06, 95% CI(-0.98 ~ -0.70)).
